...
#import "MyShopAddJobViewController.h"
#import "Ty_Model_WorkListInfo.h"
#import "MyShopAddServicesViewController.h"//添加工种界面

@interface MyShopAddJobViewController ()
{
...
@implementation MyShopAddJobViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView==tableView2_) {
        if ([arrContent2 count]>0) {
            ...
            MyShopAddServicesViewController * myShopAddServices = [[MyShopAddServicesViewController alloc]init];
            myShopAddServices.myShopAddSkill = self.myShopAddSkill;
            myShopAddServices.WorklistModel.workName = [[arrContent2 objectAtIndex:indexPath.row] objectForKey:@"workName"];
            myShopAddServices.WorklistModel.workGuid = [[arrContent2 objectAtIndex:indexPath.row] objectForKey:@"workGuid"];
            [self.naviGationController pushViewController:myShopAddServices animated:YES];
